// backend/src/validators/common.ts - VERSION UNIFIÉE (enums Prisma)
import { z } from "zod";
import {
  Role,
  SeedLevel,
...
export const positiveIntSchema = z
  ...
  .transform((val) => {
    const num = typeof val === "string" ? parseInt(val, 10) : val;
    if (isNaN(num) || num <= 0) throw new Error("Doit être un entier positif");
    return num;
  });
...
export const positiveIdSchema = z.union([
  ...
  z.string().transform((val) => {
    ...
    if (isNaN(parsed) || parsed <= 0) throw new Error("ID invalide");
    return parsed;
  }),
]);
...
export const ValidationUtils = {
  validateLotId: (lotId: string): boolean =>
    /^SL-(GO|G[1-4]|R[1-2])-\d{4}-\d{3}$/.test(lotId),
  isInSenegal: (lat: number, lng: number) =>
    lat >= 12.0 && lat <= 16.7 && lng >= -17.6 && lng <= -11.3,
  isValidCoordinates: (lat: number, lng: number) =>
    lat >= -90 && lat <= 90 && lng >= -180 && lng <= 180,
  isValidPhoneNumber: (phone: string) =>
    /^(\+221)?[0-9]{8,9}$/.test(phone.replace(/\s+/g, "")),
  isValidEmail: (email: string) => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email),
  isValidSeedLevel: (level: string) =>
    Object.values(SeedLevel).includes(level as SeedLevel),
